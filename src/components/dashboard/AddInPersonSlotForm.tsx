...
export default function AddInPersonSlotForm({
  ...
  useEffect(() => {
    if (isEditMode && initialData && subjects.length > 0) {
      ...
      if (
        initialData.days &&
        Array.isArray(initialData.days) &&
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      ...
      addToast({
        ...
        description:
          "Please fill in the subject, title, date range, and price to create your in-person session.",
        duration: 5000,
      });
      return;
    }
    ...
    try {
      if (isEditMode && slotId) {
        ...
        addToast({
          ...
          description:
            result?.message ||
            `Your in-person session has been updated successfully. ${slotsCount} ${slotsText} are now available for booking.`,
          duration: 6000,
        });
        ...
        addToast({
          ...
          description:
            result?.message ||
            "Your in-person session is now available for students to book.",
          duration: 5000,
        });
        resetForm();
      }
      ...
      if (error instanceof Error) {
        ...
        if (errorText.includes("network") || errorText.includes("fetch")) {
          ...
          errorMessage =
            "Your video file is too large. Please compress it or use a file under 50MB.";
        } else if (errorText.includes("format") || errorText.includes("type")) {
          errorMessage =
            "Your video file format is not supported. Please use MP4, WebM, or Ogg format.";
                    ...
                    <div className="space-y-4 mt-8 animate-in fade-in duration-500">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-3">
                        {daySlot.times.map((timeSlot, timeIndex) => (
                          <div
                            key={timeIndex}
                            className="bg-gray-800/30 p-3 rounded-lg border border-gray-700/50 flex flex-col lg:flex-row lg:items-center justify-between gap-3"
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <div className="flex items-center gap-4 text-xs">
                                <div className="flex items-center gap-2">
                                  ...
                                  <Input
                                    ...
                                    className="h-8 w-24 bg-gray-700/30 border-0 p-1 text-center text-white"
                                  />
                                </div>
                                <div className="flex items-center gap-2">
