...
import { InPersonSlot } from '@/hooks/slots/use-in-person-slots'
import { Loader2 } from 'lucide-react'

export default function EditInPersonSessionPage() {
  const [user, setUser] = useState<{ id: number; name: string; email: string } | null>(null)
  const [loading, setLoading] = useState(true)
  const [slotData, setSlotData] = useState<InPersonSlot | null>(null)
  ...
  const { data: firstPageData } = useInPersonSlots(1)
      ...
      while (page <= maxPages) {
        try {
          ...
          if (response.ok && result?.success && result?.slots?.data) {
            const foundSlot = result.slots.data.find((s: InPersonSlot) => s.id === slotId)
            if (foundSlot) {
  ...
  return (
    <DashboardLayout>
      <AddInPersonSlotForm slotId={slotId} initialData={slotData} />
    </DashboardLayout>
  )
}
