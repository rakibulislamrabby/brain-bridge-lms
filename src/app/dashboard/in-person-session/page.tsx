...
import { useInPersonSlots } from '@/hooks/slots/use-in-person-slots'
...
export default function InPersonSessionPage() {
  ...
  const { data: paginatedData, isLoading: slotsLoading, error: slotsError } = useInPersonSlots(currentPage)
  const deleteSlotMutation = useDeleteInPersonSlot()
  ...
  const bookedSlots = useMemo(() => slots.filter((slot) => slot.is_booked).length, [slots])
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-white">In-Person Sessions</h1>
            <p className="text-gray-400 mt-2">Manage and monitor all your in-person teaching slots.</p>
          </div>
          <Button asChild className="bg-orange-600 hover:bg-orange-700 text-white cursor-pointer">
            <Link href="/dashboard/in-person-session/add-slot">
        ...
        <Card className="bg-gray-800 border-gray-700">
          <CardHeader>
            ...
            <CardDescription className="text-gray-400">
              Overview of all in-person slots
            </CardDescription>
          </CardHeader>
            ...
            {slotsLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 space-y-4">
                ...
                <p className="text-gray-400">
                  {slotsError instanceof Error ? slotsError.message : 'Failed to load in-person slots. Please try again.'}
                </p>
              </div>
            ) : slots.length === 0 ? (
              <div className="py-12 text-center text-gray-400">
                No in-person slots available yet. Click &quot;Add Slot&quot; to create your first slot.
              </div>
            ) : (
        ...
        <Dialog open={deleteDialogOpen} onOpenChange={(open: boolean) => {
          ...
          <DialogContent className="bg-gray-800 border-gray-700">
            <DialogHeader>
              <DialogTitle className="text-white">Delete In-Person Slot</DialogTitle>
              <DialogDescription className="text-gray-400">
                Are you sure you want to delete this in-person slot? This action cannot be undone.
              </DialogDescription>
            </DialogHeader>
            {slotToDelete && (
              <div className="mt-4">
                <p className="text-gray-300">
                  <span className="font-medium">Slot:</span> {slotToDelete.label}
                </p>
              </div>
            )}
            <DialogFooter className="flex flex-col sm:flex-row sm:justify-end sm:space-x-3 gap-3 mt-6">
              <Button
                variant="outline"
                onClick={() => {
                  setDeleteDialogOpen(false)
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    addToast({
                      title: 'Slot deleted',
                      description: `In-person slot "${slotToDelete.label}" has been removed.`,
                      type: 'success',
                    })
                    setDeleteDialogOpen(false)
                    setSlotToDelete(null)
                    ...
                    addToast({
                      ...
                      description: error instanceof Error ? error.message : 'Failed to delete in-person slot.',
                      type: 'error',
                    })
                  } finally {
                    setDeletingId(null)
